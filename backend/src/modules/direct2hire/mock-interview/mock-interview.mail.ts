...
function infoCardHtml(data: MockInterviewScheduleEmailData): string {
  ...
  return `
        <table role="presentation" cellpadding="0" cellspacing="0" style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px 20px; margin-bottom: 28px; width: 100%; table-layout: fixed;">
            ${row("Interviewer Name", data.interviewerName)}
            ${row("Interview Date", data.date)}
            ${row("Interview Time", data.time)}
            ...
            <tr>
                <td style="padding: 12px 0 0; vertical-align: top; word-break: break-all; word-wrap: break-word; overflow-wrap: break-word;">
                    <span style="display: block; color: #64748b; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Google Meet Link</span>
...
function wrapEmailHtml(
  heading: string,
  intro: string,
  data: MockInterviewScheduleEmailData,
): string {
  return `
                        ...
                        <div style="padding: 32px 24px;">
                            ...
                            <div style="margin-bottom: 28px;">
                                ...
                                <ul style="color: #475569; font-size: 13.5px; line-height: 1.7; margin: 0; padding-left: 20px;">
                                    <li style="margin-bottom: 6px;">Please join the meeting 10 minutes before the scheduled time.</li>
                                    <li style="margin-bottom: 6px;">Ensure you have a stable internet connection.</li>
                                    <li style="margin-bottom: 6px;">Keep your resume ready for reference.</li>
                                    <li style="margin-bottom: 0;">Keep your camera and microphone ready and tested.</li>
                                </ul>
                            </div>
                            ...
                            <p style="color: #94a3b8; font-size: 12px; text-align: center; margin: 0 0 8px; line-height: 1.6;">
                                If you cannot attend the interview, please contact the Avatar India support team beforehand.
                            </p>
                            <p style="color: #94a3b8; font-size: 12px; text-align: center; margin: 0; line-height: 1.6;">
                                Thank you,<br />
                                <span style="font-weight: 600; color: #64748b;">Avatar India Team</span>
                            </p>
                        </div>
...
export function buildMockInterviewScheduledEmail(
  data: MockInterviewScheduleEmailData,
): BuiltEmail {
  return {
    subject: "Your Mock Interview has been Scheduled | Avatar India",
    html: wrapEmailHtml(
      "Mock Interview Scheduled",
      "Congratulations! Your Pre-Placement Mock Interview has been scheduled. Please review the details below and come prepared.",
      data,
    ),
  };
}

export function buildMockInterviewUpdatedEmail(
  data: MockInterviewScheduleEmailData,
): BuiltEmail {
  return {
    subject: "Your Mock Interview has been Rescheduled | Avatar India",
    html: wrapEmailHtml(
      "Mock Interview Rescheduled",
      "Your mock interview details have been updated. Please find the latest schedule below and come prepared.",
      data,
    ),
  };
}

export async function sendMockInterviewScheduleEmail(
  email: string,
  data: MockInterviewScheduleEmailData,
  ...
  const { subject, html } =
    kind === "scheduled"
      ? buildMockInterviewScheduledEmail(data)
      : buildMockInterviewUpdatedEmail(data);

  console.log(`[Resend] Sending mock interview ${kind} email to: ${email}`);
  try {
    ...
    if (error) {
      console.error(
        `[Resend Error] Failed to send mock interview ${kind} email to ${email}:`,
        error,
      );
      throw error;
    }
    ...
    console.log(
      `[Resend Success] Mock interview ${kind} email sent! ID: ${sendData?.id}`,
    );
    return sendData;
  } catch (error: unknown) {
    console.error(
      `[Resend] Error sending mock interview ${kind} email to ${email}:`,
      error,
    );
    throw error;
  }
