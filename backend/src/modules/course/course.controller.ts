...
export const adminGetCourse = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const togglePublish = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        sendResponse(res, true, course, `Course ${course.isPublished ? "published" : "unpublished"}`);
    } catch (err: any) {
        sendResponse(res, false, null, err.message, err.statusCode ?? STATUS_CODES.SERVER_ERROR);
    }
};
...
export const createLesson = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const lesson = await adminService.createLesson(param(req, "courseId"), value);
...
export const updateLesson = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const lesson = await adminService.updateLesson(param(req, "lessonId"), value);
...
export const initVideoUpload = async (req: Request, res: Response): Promise<void> => {
    try {
        const { title } = req.body as { title?: string };
        if (!title?.trim()) {
            sendResponse(res, false, null, "title is required", STATUS_CODES.BAD_REQUEST);
            return;
        }
        const result = await adminService.initVideoUpload(param(req, "lessonId"), title.trim());
...
export const completeVideoUpload = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!videoGuid || !title) {
            sendResponse(res, false, null, "videoGuid and title are required", STATUS_CODES.BAD_REQUEST);
            return;
        }
...
export const enrollCourse = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const enrollment = await publicService.enrollUser(param(req, "courseId"), req.user!.id);
        sendResponse(res, true, enrollment, "Enrolled successfully", STATUS_CODES.CREATED);
    } catch (err: any) {
        sendResponse(res, false, null, err.message, err.statusCode ?? STATUS_CODES.SERVER_ERROR);
    }
};

export const unenrollCourse = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
...
export const getEnrolledCourseDetail = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const course = await publicService.getEnrolledCourseDetail(param(req, "courseId"), req.user!.id);
        sendResponse(res, true, course, "Course fetched");
    } catch (err: any) {
        sendResponse(res, false, null, err.message, err.statusCode ?? STATUS_CODES.SERVER_ERROR);
    }
...
export const checkEnrollment = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const result = await publicService.checkEnrollment(param(req, "courseId"), req.user!.id);
...
export const downloadResource = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { filename, contentType, contentLength, stream } = await publicService.streamResourceDownload(
        ...
        stream.on("error", () => {
            if (!res.headersSent) {
                sendResponse(res, false, null, "Download failed", STATUS_CODES.SERVER_ERROR);
            } else {
                res.end();
            }
        });
