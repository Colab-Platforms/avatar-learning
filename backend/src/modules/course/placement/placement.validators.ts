import Joi from "joi";
import { DEFAULT_MAX_ATTEMPTS, DEFAULT_QUESTIONS_PER_ATTEMPT } from "./placement.types.js";
...
const createPlacementAssessmentSchema = Joi.object({
  ...
  questionsPerAttempt: Joi.number().integer().min(1).default(DEFAULT_QUESTIONS_PER_ATTEMPT),
  maxTabSwitchWarnings: Joi.number().integer().min(1).default(3),
  maxAttempts: Joi.number().integer().min(1).default(DEFAULT_MAX_ATTEMPTS),
});
