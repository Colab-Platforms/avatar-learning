...
const createAssessmentSchema = Joi.object({
  ...
  lessonId: Joi.string().trim().optional().allow(null, ""),
  ...
  maxAttempts: Joi.number().integer().min(1).optional().allow(null),
}).custom((value, helpers) => {
  if (value.type === "WEEKLY" && !value.lessonId) {
    return helpers.message({ custom: "Weekly assessments require a lessonId" });
  }
  if (value.type === "FINAL" && value.lessonId) {
    return helpers.message({ custom: "Final assessments must not have a lessonId" });
  }
  return value;
});
...
export const validateCreateAssessment = (data: unknown) =>
  createAssessmentSchema.validate(data, { abortEarly: false });
export const validateUpdateAssessment = (data: unknown) =>
  updateAssessmentSchema.validate(data, { abortEarly: false });
export const validateCreateQuestion = (data: unknown) =>
  createQuestionSchema.validate(data, { abortEarly: false });
export const validateUpdateQuestion = (data: unknown) =>
  updateQuestionSchema.validate(data, { abortEarly: false });
export const validateSaveAnswer = (data: unknown) =>
  saveAnswerSchema.validate(data, { abortEarly: false });
