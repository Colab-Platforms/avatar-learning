import Joi from "joi";
import type { CreateOrderBody, VerifyPaymentBody } from "./payment.types.js";
...
export function validateVerifyPayment(data: unknown): {
  ...
  const schema = Joi.object<VerifyPaymentBody>({
  ...
  const { error, value } = schema.validate(data, { abortEarly: true });
  if (error) return { error: { message: error.message }, value: value as VerifyPaymentBody };
  return { value };
}
