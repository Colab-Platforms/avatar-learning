...
import { ArrowRight, BookOpen, Briefcase, CheckCircle2, Loader2, Mail } from "lucide-react";
...
export default function CourseSuccessPage({ params }: PageProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (enrolled && !firedRef.current) {
      ...
      window.fbq?.("track", "Purchase", { value: amount ?? undefined, currency: "INR" });
    }
  }, [enrolled, amount]);
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-6 py-16">
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {/* Top accent */}
          <div className={`h-1.5 w-full ${isD2H ? "bg-gradient-to-r from-emerald-500 to-teal-500" : "bg-gradient-to-r from-blue-500 to-indigo-500"}`} />

          <div className="p-8 text-center">
            {/* Success icon */}
            <div className={`mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full ${isD2H ? "bg-emerald-50 border border-emerald-200" : "bg-blue-50 border border-blue-200"}`}>
              <CheckCircle2 className={`h-10 w-10 ${isD2H ? "text-emerald-500" : "text-blue-500"}`} />
            </div>
            ...
            <div className="mt-6 rounded-xl border border-slate-100 bg-slate-50 text-left overflow-hidden">
              <div className="px-4 py-2.5 bg-slate-100 border-b border-slate-200">
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Order Summary</span>
              </div>

              <div className="divide-y divide-slate-100">
                {/* Course */}
                <div className="flex items-start justify-between px-4 py-3 gap-3">
                  <span className="text-sm text-slate-500 shrink-0">Course</span>
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  ...
                  <span className={`text-sm font-semibold px-2.5 py-0.5 rounded-full ${isD2H ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>
                    {planLabel}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  ...
                  <span className="text-sm font-medium text-slate-700">{purchaseDate}</span>
                </div>
            ...
            {/* Email notice */}
            <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50 p-3.5 text-left">
              <Mail className="h-4 w-4 shrink-0 text-blue-600 mt-0.5" />
              <p className="text-xs text-blue-800 leading-relaxed">
                A payment confirmation has been sent to your email. Check your inbox (and spam folder).
              </p>
            </div>
            ...
            <Link
              href={`/dashboard/${id}`}
              className={`mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl px-6 py-3.5 text-sm font-semibold text-white transition shadow-lg ${isD2H ? "bg-gradient-to-r from-emerald-600 to-teal-600 hover:brightness-110 shadow-emerald-600/20" : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:brightness-110 shadow-blue-600/20"}`}
            >
