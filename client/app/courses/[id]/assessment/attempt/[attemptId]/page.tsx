...
import { useAssessmentAttempt } from "@/hooks/queries/useAssessmentAttempt";
import { useSaveAssessmentAnswer } from "@/hooks/mutations/useSaveAssessmentAnswer";
import { useReportTabSwitchViolation } from "@/hooks/mutations/useReportTabSwitchViolation";
import { useSubmitAssessmentAttempt } from "@/hooks/mutations/useSubmitAssessmentAttempt";
...
export default function AssessmentAttemptPage({ params }: PageProps) {
  ...
  const { data, isLoading, isError } = useAssessmentAttempt(attemptId);
  const saveAnswer = useSaveAssessmentAnswer(attemptId);
  const reportViolation = useReportTabSwitchViolation(attemptId);
  const submitAttempt = useSubmitAssessmentAttempt(attemptId);
  ...
  // Sorted questions — memoized so it's stable across renders
  ...
  useEffect(() => {
    if (attempt && attempt.status !== "IN_PROGRESS" && !autoSubmitReason) {
      router.replace(`/courses/${id}/assessment/results/${attemptId}`);
    }
  }, [attempt, autoSubmitReason, router, id, attemptId]);
  ...
  }, [inProgress, submitAttempt, router, id, attemptId]);

  // Optimistic local answers so UI feels instant
  const [localAnswers, setLocalAnswers] = useState<Record<string, string | null>>({});

  // Merge server answers + local optimistic answers
  ...
  const handleManualSubmit = () => {
    submitAttempt.mutate(undefined, {
      onSuccess: () => router.replace(`/courses/${id}/assessment/results/${attemptId}`),
    });
  };
  ...
  return (
    <>
      ...
      >
        {/* Two-column layout: question on left, navigator panel on right */}
