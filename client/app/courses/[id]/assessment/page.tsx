...
export default function PublicAssessmentHubPage({ params }: PageProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    if (inD2H) router.replace(d2hLearningRoutes(id).assessments);
  ...
  return (
    <>
      <Navbar />
      <main className="min-h-screen mt-8 bg-gradient-to-b from-slate-50 to-white">
        <LearningRouteProvider courseId={id} scope="public">
          <AssessmentHubView courseId={id} />
