...
import {
  ...
  ChevronDown, ArrowRight, Download, CheckCircle, Loader2,
} from "lucide-react";
import { Badge, Button, ScrollReveal, AnimateOnScroll } from "@/components/ui";
import { Navbar } from "@/components/layout/Navbar";
import { Footer } from "@/components/layout/Footer";
import {
  fetchCourseBySlug, enrollCourse, checkEnrollment,
  type DBCourseDetail,
} from "@/lib/coursesApi";
...
export default function CoursePage({ params }: PageProps) {
  ...
  const [course, setCourse]             = useState<DBCourseDetail | null>(null);
  ...
  // Check enrollment status when course + user are loaded
  useEffect(() => {
  ...
  const handleEnroll = useCallback(async () => {
    ...
    if (enrolled) { router.push(`/courses/${id}/learn`); return; }
  ...
  }, [user, enrolled, course, id, router]);

  if (loading) return (
    <>
      <Navbar />
      <main className="min-h-screen" style={{ background: "linear-gradient(160deg, #060D1A 0%, #091220 25%, #060D1A 50%, #091525 75%, #060D1A 100%)" }}>
        <div className="h-[340px] sm:h-[440px] md:h-[520px] bg-ink-800 animate-pulse" />
        <div className="container-x pt-10 space-y-6">
          {[...Array(4)].map((_, i) => (
            <div key={i} className={`rounded-2xl bg-ink-800 animate-pulse ${i === 0 ? "h-32" : "h-20"}`} />
          ))}
        </div>
      </main>
      <Footer />
  ...
  const weekData     = [...course.lessons].sort((a, b) => a.weekNumber - b.weekNumber);
  const whatYouLearn = course.whatYouLearn ?? [];
  const audience     = course.audience ?? [];

  const enrollBtnLabel = enrolling ? "Enrolling…"
  ...
  return (
    <>
      ...
      <main className="min-h-screen text-white overflow-x-hidden"
        style={{ background: "linear-gradient(160deg, #060D1A 0%, #091220 25%, #060D1A 50%, #091525 75%, #060D1A 100%)" }}
      >
        {/* ── BANNER ── */}
        <div className="relative w-full h-[340px] sm:h-[440px] md:h-[520px] overflow-hidden">
          {course.bannerImage ? (
            <Image src={course.bannerImage} alt={course.title} fill sizes="100vw"
              className="object-cover object-center scale-[1.04] transition-transform duration-[8000ms] ease-out"
              style={{ transformOrigin: "center 40%" }} priority />
          ) : (
            <div className="absolute inset-0"
              style={{ background: "linear-gradient(135deg, rgba(0,60,140,0.5) 0%, rgba(0,200,255,0.1) 100%)" }} />
          )}
            ...
            <div className="grid md:grid-cols-[1fr_380px] gap-14 items-start">
              ...
              <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    ...
                    {course.certificate && (
                      <Badge variant="level-dark" className="flex items-center gap-1">
                        <BadgeCheck className="h-3 w-3 text-emerald-400" /> Certificate Included
                      </Badge>
                    )}
                    {enrolled && (
                ...
                <ScrollReveal animation="fade-up" delay={180} duration={750}>
                  <p className="text-white/55 text-[16px] leading-relaxed max-w-xl mb-6">{course.description}</p>
                </ScrollReveal>
                    ...
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm mb-6">
                      {course.rating && (
                        <span className="flex items-center gap-1.5">
                          {[1,2,3,4,5].map((s) => (
                            <Star key={s} className={`h-4 w-4 ${s <= Math.round(course.rating!) ? "fill-amber-400 text-amber-400" : "text-white/20"}`} />
                          ))}
                          <span className="ml-1 font-semibold text-white">{course.rating}</span>
                          {course.reviews && <span className="text-white/40">({course.reviews})</span>}
                        </span>
                      )}
                      {course.sessions && <span className="flex items-center gap-1.5 text-white/45"><Clock className="h-4 w-4" />{course.sessions}</span>}
                      {course.seats    && <span className="flex items-center gap-1.5 text-white/45"><Users className="h-4 w-4" />{course.seats}</span>}
                    </div>
                  </ScrollReveal>
                )}
                ...
                <ScrollReveal animation="fade-up" delay={280} duration={700}>
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px] text-white/45 border-t border-white/6 pt-5 mb-8">
                    {course.startDate && <span><span className="text-white/25 mr-1">Starts:</span><span className="text-white/65 font-medium">{course.startDate}</span></span>}
                    <span><span className="text-white/25 mr-1">Duration:</span><span className="text-white/65 font-medium">{course.totalWeeks} Weeks</span></span>
                    <span><span className="text-white/25 mr-1">Mode:</span><span className="text-white/65 font-medium">Live + Recorded</span></span>
                  </div>
                </ScrollReveal>
                ...
                {enrollMsg && (
                  <ScrollReveal animation="fade-up" delay={0} duration={400}>
                    <div className={`mb-4 rounded-xl border px-4 py-3 text-sm flex items-center gap-2 ${
                      ...
                      {enrollMsg.includes("enrolled") ? <CheckCircle className="h-4 w-4 shrink-0" /> : null}
                      {enrollMsg}
                    </div>
                  </ScrollReveal>
                )}
            ...
            <div className="relative container-x">
              <ScrollReveal animation="fade-up" duration={700}>
                <div className="text-center mb-14">
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-brand-400 mb-3">Curriculum Overview</p>
                  <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">What You&apos;ll Learn</h2>
                  <p className="mt-3 text-white/40 max-w-xl mx-auto">Practical real-world skills you can apply in your work immediately.</p>
                </div>
              </ScrollReveal>
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {whatYouLearn.map((item, i) => (
                  <AnimateOnScroll key={i} delay={i * 100}>
                    <div className="group h-full rounded-2xl border border-white/6 p-6 cursor-default hover:border-brand-500/30 hover:-translate-y-1.5 transition-all duration-350"
                      style={{ background: "linear-gradient(145deg, rgba(13,23,39,0.85) 0%, rgba(9,18,32,0.95) 100%)", boxShadow: "0 4px 24px rgba(0,0,0,0.30)" }}>
                      <h3 className="font-semibold text-[15px] text-white mb-2 leading-snug group-hover:text-brand-300 transition-colors duration-300">{item.title}</h3>
        ...
        {weekData.length > 0 && (
          <section className="py-24 border-t border-white/5 relative overflow-hidden">
            ...
            <div className="relative container-x">
              <ScrollReveal animation="fade-up" duration={700}>
                <div className="text-center mb-14">
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-brand-400 mb-3">Curriculum</p>
                  <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Program Structure</h2>
                  <p className="mt-3 text-white/40 max-w-xl mx-auto">A structured week-by-week path — each milestone builds directly on the last.</p>
                </div>
              </ScrollReveal>
                ...
                {weekData.map((week, i) => (
                  <AnimateOnScroll key={week.id} delay={i * 80}>
                    <div className="rounded-2xl border overflow-hidden transition-all duration-300"
                      style={{ borderColor: openWeek === i ? "rgba(0,200,255,0.20)" : "rgba(255,255,255,0.06)",
                               background: openWeek === i ? "linear-gradient(145deg, rgba(0,200,255,0.04) 0%, rgba(9,18,32,0.98) 100%)" : "linear-gradient(145deg, rgba(13,23,39,0.85) 0%, rgba(9,18,32,0.95) 100%)" }}>
                      <button onClick={() => setOpenWeek(openWeek === i ? null : i)}
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-4">
                          <span className="text-[12px] text-white/30 hidden sm:inline">{week.modules.length} modules</span>
                          <ChevronDown className={`h-5 w-5 text-white/40 transition-transform duration-400 ${openWeek === i ? "rotate-180 text-brand-400" : ""}`} />
                        </div>
                      </button>
                      <div className="overflow-hidden transition-all duration-500" style={{ maxHeight: openWeek === i ? "600px" : "0px" }}>
                        <div className="px-6 pb-5 border-t border-white/5">
                          <ul className="mt-4 space-y-3">
                            {week.modules.map((mod, j) => (
                              <li key={j} className="flex items-start gap-3 text-[14px] text-white/55 hover:text-white/80 transition-colors duration-200">
                                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500/8 text-brand-400 text-[10px] font-semibold">{j + 1}</span>
                                {mod}
                              </li>
                            ))}
                          </ul>
            ...
            <div className="relative container-x">
              <ScrollReveal animation="fade-up" duration={700}>
                <div className="text-center mb-14">
                  ...
                  <p className="mt-3 text-white/40 max-w-xl mx-auto">Designed for anyone ready to thrive in an AI-powered world.</p>
                </div>
              </ScrollReveal>
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {audience.map((item, i) => (
                  <AnimateOnScroll key={i} delay={i * 100}>
                    <div className="group h-full rounded-2xl border border-white/6 p-6 cursor-default hover:border-brand-500/30 hover:-translate-y-1.5 transition-all duration-350"
                      style={{ background: "linear-gradient(145deg, rgba(13,23,39,0.85) 0%, rgba(9,18,32,0.95) 100%)", boxShadow: "0 4px 24px rgba(0,0,0,0.25)" }}>
                      <h3 className="font-semibold text-[15px] text-white mb-2 group-hover:text-brand-300 transition-colors duration-300">{item.title}</h3>
                      <p className="text-[13px] text-white/40 leading-relaxed">{item.body}</p>
                    </div>
                  </AnimateOnScroll>
                ))}
          ...
          <div className="container-x">
            <ScrollReveal animation="zoom-in" duration={800}>
              <div className="relative rounded-3xl overflow-hidden border border-brand-500/20 p-10 sm:p-16 text-center"
                style={{ background: "linear-gradient(135deg, rgba(9,21,37,0.97) 0%, rgba(6,13,26,0.99) 100%)",
                ...
                <p className="relative text-[11px] font-semibold uppercase tracking-widest text-brand-400 mb-3">
                  {enrolled ? "Continue Learning" : isFree ? "100% Free" : "Enrol Now"}
                </p>
                <h2 className="relative text-3xl sm:text-4xl font-bold mb-4">
                  {enrolled ? "You're enrolled — let's go!" : "Ready to start your AI journey?"}
                </h2>
                ...
                <div className="relative flex flex-wrap justify-center gap-3">
                  <button onClick={handleEnroll} disabled={enrolling || (course.price > 0 && !enrolled)}
                    className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl text-sm font-semibold
                               ...
                               shadow-[0_4px_20px_rgba(0,200,255,0.3)]">
                    {enrolling && <Loader2 className="h-4 w-4 animate-spin" />}
                    {enrolled ? "Go to Course" : isFree ? "Enroll for Free" : "Enroll Now"}
