...
import {
  ChevronDown, Video, FileText, Download, Loader2,
...
function resourceIcon(category: string) {
  if (category === "VIDEO") return <Video size={15} className="text-brand-400 shrink-0" />;
...
function ResourceCard({ res }: { res: DBResource }) {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const ext = isVideo ? "mp4" : res.url.match(/\.([a-z0-9]{2,5})(?:[?#]|$)/i)?.[1]?.toLowerCase();
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-white/6 bg-ink-900/60 px-4 py-3
                    hover:border-brand-500/25 hover:bg-ink-900/80 transition-all duration-200 group">
      {/* icon */}
      <div className="h-9 w-9 rounded-lg bg-ink-800 border border-white/8 flex items-center justify-center shrink-0
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-white/85 truncate">{res.title}</p>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[10px] text-white/30 uppercase tracking-wider">
            {isVideo ? "Video" : res.category.charAt(0) + res.category.slice(1).toLowerCase()}
          </span>
...
export default function LearnPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    fetchEnrolledCourseDetail(id)
      .then((data) => {
        setCourse(data);
        if (data.lessons.length > 0) setOpenWeek(data.lessons[0].id);
      })
      .catch((err) => {
        const msg    = err?.response?.data?.message ?? "Failed to load course.";
        const status = err?.response?.status;
        if (status === 403) router.replace(`/courses/${id}`);
        else setError(msg);
      })
  ...
  if (loading) return (
    <>
      <Navbar />
      <main className="min-h-screen" style={{ background: "linear-gradient(160deg,#060D1A 0%,#091220 35%,#060D1A 100%)" }}>
        <div className="container-x py-12 space-y-4 max-w-4xl">
          {[...Array(5)].map((_, i) => (
            <div key={i} className={`rounded-2xl bg-ink-800 animate-pulse ${i === 0 ? "h-28" : "h-20"}`} />
          ))}
        </div>
      </main>
      <Footer />
  ...
  if (error || !course) return (
    <>
      ...
      <main className="min-h-screen flex items-center justify-center"
        style={{ background: "linear-gradient(160deg,#060D1A 0%,#091220 35%,#060D1A 100%)" }}>
        <div className="text-center">
          <p className="text-red-400 mb-4">{error || "Course not found."}</p>
          <Link href="/courses" className="text-brand-400 text-sm hover:text-brand-300">← Back to courses</Link>
        </div>
      </main>
      <Footer />
    </>
  ...
  const totalResources = sortedLessons.reduce((acc, l) => acc + l.resources.length, 0);

  return (
    <>
      <Navbar />
      <main className="min-h-screen mt-8 text-white"
        style={{ background: "linear-gradient(160deg,#060D1A 0%,#091220 35%,#060D1A 100%)" }}>
        <div className="container-x py-10 max-w-4xl">
          ...
          <div className="flex items-center gap-2 text-xs text-white/35 mb-6">
            ...
            <Link href={`/courses/${id}`} className="hover:text-white/60 transition-colors truncate max-w-48">
              {course.title}
            </Link>
            <ChevronRight size={12} />
          ...
          {/* Course header card */}
          <div className="rounded-2xl border border-white/6 bg-ink-800 p-6 mb-8"
            style={{ boxShadow: "inset 0 1px 0 rgba(0,200,255,0.06)" }}>
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <CheckCircle size={14} className="text-emerald-400 shrink-0" />
                  <span className="text-xs text-emerald-400 font-semibold uppercase tracking-wider">Enrolled</span>
                </div>
                ...
                <div className="flex flex-wrap gap-4 text-xs text-white/40 mt-2">
                  ...
                  <span className="flex items-center gap-1.5"><Package size={12} />{totalResources} resources</span>
                  <span className="flex items-center gap-1.5"><FileText size={12} />{sortedLessons.length} lessons</span>
                </div>
              </div>
              <div className="shrink-0 text-right">
                <p className="text-[11px] text-white/25 uppercase tracking-wider mb-1">Since</p>
                <p className="text-sm text-white/60">
                  {new Date(course.enrollment.enrolledAt).toLocaleDateString("en-IN", {
          ...
          {totalResources === 0 && (
            <div className="rounded-2xl border border-white/5 bg-ink-800 py-16 text-center">
              ...
              <p className="text-sm text-white/35">No resources have been added yet.</p>
              <p className="text-xs text-white/20 mt-1">Check back soon — new material will appear here.</p>
            </div>
          )}
              ...
              {sortedLessons.map((lesson: DBLesson) => {
                ...
                return (
                  <div key={lesson.id}
                    ...
                    style={{ borderColor: isOpen ? "rgba(0,200,255,0.18)" : "rgba(255,255,255,0.06)" }}>
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg bg-brand-500/10 border border-brand-500/15
                                      flex items-center justify-center shrink-0">
                        <span className="text-xs font-bold text-brand-400">{lesson.weekNumber}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-white/90 leading-snug">{lesson.title}</p>
                        {lesson.description && (
                          <p className="text-xs text-white/35 mt-0.5 line-clamp-1">{lesson.description}</p>
                        )}
                      </div>
