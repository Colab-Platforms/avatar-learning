...
export function PlacementOptionListEditor({
  options,
  onChange,
}: {
  options: PlacementOptionInput[];
  onChange: (o: PlacementOptionInput[]) => void;
}) {
  const update = (i: number, patch: Partial<PlacementOptionInput>) =>
...
export function PlacementQuestionRow({ question, onChanged }: { question: AdminPlacementQuestion; onChanged: () => void }) {
  ...
  const submitEdit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await updatePlacementQuestion(question.id, form);
      setEditing(false);
      onChanged();
    } catch (err: unknown) {
