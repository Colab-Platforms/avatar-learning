import { ChevronDown, Video, FileText, Trash2, Upload, Pencil } from "lucide-react";
...
export function WeekRow({
    lesson, isExpanded, isEditingModules,
    onToggleExpand, onEditModules, onModulesSaved,
    onDelete, isDeleting,
    onDeleteResource, deletingResource,
    uploadingFor, onStartUpload, onUploadDone, onCancelUpload,
}: {
    lesson: Lesson;
    isExpanded: boolean;
    ...
    return (
        <div className="px-6 py-5">
            ...
            <div className="flex items-start justify-between gap-4">
                <button onClick={onToggleExpand} className="flex items-start gap-3 min-w-0 flex-1 text-left">
                    ...
                    <div className="min-w-0">
                        ...
                        {lesson.description && (
                            <p className="text-xs text-white/40 mt-0.5 line-clamp-1">{lesson.description}</p>
                        )}
                        <div className="flex items-center gap-2 mt-1.5">
                            {lesson.isFreePreview && (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-elec-500/10 text-elec-400 uppercase tracking-wide">Free</span>
                            )}
                            <span className="text-[10px] text-white/25">{lesson.modules.length} topics</span>
                            <span className="text-[10px] text-white/25">{lesson.resources.length} video{lesson.resources.length !== 1 ? "s" : ""}</span>
                        </div>
                    </div>
                </button>
                ...
                <div className="mt-4 ml-11 space-y-4">
                    ...
                    <div className="border border-white/5 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 bg-ink-900/40 border-b border-white/5">
                            <p className="text-[11px] font-semibold text-white/40 uppercase tracking-widest">Lesson Topics</p>
                            {!isEditingModules && (
                                <button onClick={onEditModules}
                        ...
                        <div className="px-4 py-3">
                            {isEditingModules ? (
                                <InlineModuleEditor lessonId={lesson.id} modules={lesson.modules} onSaved={onModulesSaved} />
                            ) : lesson.modules.length === 0 ? (
                                <p className="text-xs text-white/25 py-1">No lesson topics yet. Click Edit to add some.</p>
                            ) : (
                    ...
                    {lesson.resources.length > 0 && (
                        <div className="space-y-1.5">
                            {lesson.resources.map((res) => (
                                <div key={res.id} className="flex items-center gap-2.5 bg-ink-900/60 border border-white/5 rounded-xl px-3.5 py-2.5">
                                    {res.category === "VIDEO"
                                    ...
                                    {res.size && <span className="text-[10px] text-white/25 shrink-0">{(Number(res.size) / 1024 / 1024).toFixed(1)} MB</span>}
                                    {res.category === "VIDEO" && (
