...
export function AssessmentEditor({ courseId }: { courseId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setNotFound(false);
    try {
      const data = await fetchAdminAssessment(courseId);
      setAssessment(data);
    } catch (err: unknown) {
  ...
  const submitCreate = async (e: React.FormEvent) => {
    ...
    try {
      await createAssessment(courseId, {
        ...
        passingScorePercent: createForm.passingScorePercent === "" ? undefined : Number(createForm.passingScorePercent),
  ...
  const submitMeta = async (e: React.FormEvent) => {
    ...
    try {
      await updateAssessment(assessment.id, {
        ...
        passingScorePercent: metaForm.passingScorePercent === "" ? null : Number(metaForm.passingScorePercent),
    ...
    return (
      <div className="bg-ink-800 border border-white/6 rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-white">Create Course Assessment</h2>
        </div>
        ...
        <form onSubmit={submitCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Passing Score (%)">
            <input
              ...
              onChange={(e) => setCreateForm((f) => ({ ...f, passingScorePercent: e.target.value }))}
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-5">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-4 text-xs text-white/40">
                ...
                {assessment.passingScorePercent != null && <span>{assessment.passingScorePercent}% to pass</span>}
