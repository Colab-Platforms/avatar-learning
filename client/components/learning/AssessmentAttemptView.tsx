...
export function AssessmentAttemptView({
  ...
  const reportViolation = useReportTabSwitchViolation(attemptId, id);
  const submitAttempt = useSubmitAssessmentAttempt(attemptId, id);
  ...
  useEffect(() => {
    if (attempt && attempt.status !== "IN_PROGRESS" && !autoSubmitReason) {
      router.replace(routes.results(attemptId));
    }
  }, [attempt, autoSubmitReason, router, routes, attemptId]);
  ...
  }, [inProgress, submitAttempt, router, routes, attemptId]);
  ...
  const handleManualSubmit = () => {
    submitAttempt.mutate(undefined, {
      onSuccess: () => router.replace(routes.results(attemptId)),
    });
  };
