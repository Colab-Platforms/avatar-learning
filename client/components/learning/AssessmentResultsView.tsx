...
export function AssessmentResultsView({
  ...
  const inCooldown =
    !!result?.summary.cooldownActive && !!result.summary.nextAttemptAvailableAt;
  ...
  const answersRevealed = result?.answersRevealed !== false;
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-500 mb-6">
              <span className="inline-flex items-center gap-1.5">
                <Clock size={12} />
                Time taken{" "}
                {formatDurationSeconds(result.attempt.durationSeconds)}
              </span>
              {result.attempt.submittedAt && (
                <span>
                  {new Date(result.attempt.submittedAt).toLocaleString(
                    undefined,
                    {
                      dateStyle: "medium",
                      timeStyle: "short",
            ...
            {exhausted && (
              <p className="text-xs text-slate-600 mb-4 rounded-xl bg-slate-50 border border-slate-100 px-3 py-2">
                Maximum attempts reached. You can still review your attempt
                history and results.
              </p>
            )}
            ...
            {!answersRevealed && (
              <p className="text-xs text-slate-600 mb-4 rounded-xl bg-slate-50 border border-slate-100 px-3 py-2">
                Question answers are hidden until you pass this assessment.
                Review your score above and retake after the cooldown if needed.
              </p>
            )}
