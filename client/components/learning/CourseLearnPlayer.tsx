...
import { useMarkIntroVideoWatched } from "@/hooks/mutations/useMarkIntroVideoWatched";
...
export function CourseLearnPlayer({ courseId }: { courseId: string }) {
  ...
  const { data: introData } = useIntroVideo({
    enabled: hasHydrated && Boolean(authUser),
  });
  const markIntroWatched = useMarkIntroVideoWatched();
  const [introPlaying, setIntroPlaying] = useState(false);
  const [introEmbedUrl, setIntroEmbedUrl] = useState<string | null>(null);
  const [viewingIntro, setViewingIntro] = useState(false);
  const hasIntro = Boolean(introData?.video);
  ...
  useEffect(() => {
    if (introAutoOpened.current || !hasIntro) return;
    introAutoOpened.current = true;
    if (!introData?.alreadyWatched) {
      setViewingIntro(true);
    }
  }, [hasIntro, introData?.alreadyWatched]);
                ...
                <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-slate-900 to-slate-950 aspect-video shadow-lg ring-1 ring-slate-900/5 shrink-0">
                  {!introData?.video ? (
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setIntroEmbedUrl(introData.video!.embedUrl);
                        setIntroPlaying(true);
                        if (!introData.alreadyWatched) {
                          markIntroWatched.mutate();
                        }
                      }}
                    ...
                    >
                      ...
                      <span className="text-sm font-medium px-6 text-center max-w-md leading-relaxed text-white/90">
                        {introData.video.title}
                      </span>
                    </button>
                  )}
                </div>

                <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-medium uppercase tracking-wider text-brand-600 mb-1.5">
                      Introduction
                    </p>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900 leading-snug">
                      Greetings From Avatar! Let's Get Started With Your
                      Learning Journey
                    </h1>
                  </div>
                  {/* {currentTopic && (
                    <button
                      ...
                      className="shrink-0 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-semibold bg-brand-600 text-white hover:bg-brand-700 transition-colors cursor-pointer shadow-sm"
                    >
                      Continue to Course
              ...
              <>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm">
                  ...
                  {currentLesson.modules &&
                    currentLesson.modules.length > 0 && (
                      <div className="mb-5 pb-5 border-b border-slate-100">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/60">
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        ...
                        {fileResources.length !== 1 ? "s" : ""} available for
                        this topic
                      </p>
                    </div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain py-1">
              {hasIntro && (
                <button
                  type="button"
                  onClick={openIntro}
                  className={`w-full flex items-center gap-3 px-4 sm:px-5 py-3.5 text-left transition-colors border-b border-slate-100 ${
                    viewingIntro
                      ? "bg-brand-50/70 hover:bg-brand-50/70"
                  ...
                  <div className="min-w-0 flex-1">
                    <p
                      className={`text-xs font-semibold leading-snug ${
                        viewingIntro ? "text-brand-700" : "text-slate-800"
                      }`}
                    >
                      Introduction
                    </p>
