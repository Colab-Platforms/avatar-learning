...
function ScoreBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-28 text-[12px] text-text-muted shrink-0">
        {domain.label}
      </span>
      ...
      <span className="text-[12px] font-semibold text-text w-9 text-right shrink-0">
        {value}%
      </span>
    </div>
  );
  ...
  return (
    <div
      className={cn(
        ...
        isTop ? "border-brand-300 bg-brand-50" : "border-border bg-white",
      )}
    >
      {/* Badge + pct */}
      <div className="relative flex items-start justify-between gap-3">
        ...
        <span
          className={cn(
            ...
            isTop ? "text-brand-600" : "text-text-muted",
          )}
        >
          {pct}%
          <span className="text-[11px] font-normal text-text-subtle ml-1">
            match
          </span>
        </span>
      </div>
      ...
      <div className="relative">
        ...
        <h3 className="text-[18px] font-semibold leading-snug text-text">
          {role.title}
        </h3>
        <p className="mt-2 text-[12px] text-text-muted leading-relaxed line-clamp-3">
          {role.desc}
        </p>
      </div>
          ...
          <QuizPanel>
            ...
            <h1 className="text-[28px] sm:text-[38px] font-semibold leading-[1.15] tracking-tight">
              <span className="text-gradient-brand">
                You are a great match for
              </span>
              <br />
              <span className="text-text">{role.title}!</span>
            </h1>
            <p className="mt-4 text-[14px] text-text-muted max-w-lg leading-relaxed">
              Based on your answers, here are the best career paths for you —
              plus a step-by-step roadmap and Avatar courses to get you started.
            </p>
          </QuizPanel>
            ...
            <div className="p-8 sm:p-10">
              ...
              <div className="relative flex flex-col gap-0">
                {role.roadmap.map((step, i) => (
                  <div
                    key={step}
                    className="flex gap-4 items-start pb-5 last:pb-0 relative"
                  >
