...
export function useDirect2HireCheckout() {
  ...
  // Ticks down while `processing` is true so the checkout page can render a
  // live countdown; mirrors the RAZORPAY_TIMEOUT_MS race in handleRazorpayCheckout.
  useEffect(() => {
    if (!processing) {
  ...
  const enroll = useCallback(
    async (lead?: Direct2HireLeadInput, couponCode?: string) => {
      ...
      if (enrolled) {
        showMessage(
          "You're already enrolled in the Direct2Hire programme.",
          "success",
        );
        return;
      }
      ...
      try {
        ...
        if (msg === "cancelled") {
          ...
          showMessage(
            "Payment is taking longer than expected. If money was deducted, it will reflect in your dashboard shortly — otherwise please retry.",
            "error",
          );
        } else {
